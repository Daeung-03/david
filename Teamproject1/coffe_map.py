...
def Serching_Analysis():
  ...
  #
    ...
    structure[structure_id_column] = structure[structure_id_column].map(category_dict)

    merged_map_struct = map.merge(structure, on=['x', 'y'], how='outer')
    merged_sort = merged_map_struct.sort_values(by='area').reset_index(drop='True')

    filtered_data = merged_sort[merged_sort['area'] == 1].reset_index(drop='True')
    return filtered_data


def Print_Report(data):
    structure_stats = data.groupby('category').agg({'x': ['count', 'min', 'max'],'y': ['min', 'max'],'ConstructionSite': 'sum'}).round(2)
